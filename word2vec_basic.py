...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #defining a collections container
  ## how large should be the window

  winsize = 2*skip_window + 1
  ## Number of available words from which we can pick
  available_word_labels = batch_size - skip_window
  ## Words required to fill the quota of samples
  words_required = (batch_size //  num_skips)
  ## uniqe chosen words in our minibatch
  labels_unique = collections.deque(maxlen = words_required)

  toggle = 0
  
  if data_index < skip_window:
    window_index = data_index + skip_window
  else:
    window_index = data_index

  if data_index + skip_window > len(data):
    print("exceeded the length of dataset......")
    toggle = 1
    
  count_samples = 0
  for i in range(words_required):
    if toggle == 1:
      break
    labels_unique.append(data[window_index])
    ## define window
    word_window = data[(window_index - skip_window):(window_index + skip_window + 1)]
    ## Number of samples to be extracted from each window
    nsamples = min(num_skips, batch_size - count_samples)
    
    for j in range(nsamples):
      predictor_word = word_window[skip_window + ((j // 2)+1)*((-1)**(j+1))]
      labels[count_samples, 0] = predictor_word
      batch[count_samples] = data[window_index]
      count_samples = count_samples + 1

    window_index = window_index + 1

  data_index = data_index + words_required
  # print("****************")
  # print(batch)
  # print(labels)
  # print(data_index)
